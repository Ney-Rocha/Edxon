      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="space-y-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white dark:bg-[#0c0c0c] rounded-3xl border border-slate-200/80 dark:border-neutral-900 p-6 shadow-sm space-y-5">
              <div className="flex items-center gap-2 border-b border-slate-100 dark:border-neutral-800 pb-3">
                <span className="p-1.5 bg-indigo-50 dark:bg-neutral-900 text-indigo-600 dark:text-indigo-400 rounded-lg">
                  <BookOpen className="h-4.5 w-4.5" />
                </span>
                <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm">Dados Básicos do Treinamento</h3>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-slate-450 dark:text-slate-300 tracking-wider">Título do Curso</label>
                <input
                  type="text"
                  required
                  value={title}
                  ...
                  className="w-full text-xs font-semibold px-4 py-2.5 rounded-xl border border-slate-200 dark:border-neutral-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-slate-50/50 dark:bg-neutral-900/60 dark:text-white dark:placeholder-neutral-500"
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-black uppercase text-slate-450 dark:text-slate-300 tracking-wider">Categoria / Trilha</label>
                    <button
                      type="button"
                      onClick={() => setShowNewTypeForm(!showNewTypeForm)}
                      className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 uppercase tracking-wider"
                    >
                  ...
                  <select
                    ...
                    className="w-full text-xs font-semibold px-4 py-2.5 rounded-xl border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 focus:ring-2 focus:ring-indigo-500 focus:outline-none dark:text-white"
                  >
                    {courseTypes.map((ct) => (
                      <option key={ct.id} value={ct.id} className="dark:bg-neutral-900">
                        {ct.name}
                      </option>
                    ))}
                    {courseTypes.length === 0 && (
                      <option value="" className="dark:bg-neutral-900">Nenhuma categoria</option>
                    )}
                  </select>

                  {/* Inline micro-creation form */}
                  {showNewTypeForm && (
                    <div className="bg-slate-50 dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-2xl p-4 space-y-3 mt-2 animate-fade-in">
                      <h4 className="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-wider">Cadastrar Nova Categoria</h4>
                      <div className="space-y-2">
                        <input
                          ...
                          className="w-full text-[11px] font-semibold px-3 py-2 rounded-lg border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900/80 dark:text-white dark:placeholder-neutral-500"
                        />
                        <input
                          type="text"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-slate-450 dark:text-slate-300 tracking-wider">Duração Estimada</label>
                  <input
                    type="text"
                    required
                    value={duration}
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black uppercase text-slate-450 dark:text-slate-300 tracking-wider block">Imagem de Capa ou Cor Customizada</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> {/* Left: upload drag selector */}
                  <div className="space-y-3">
                    <span className="text-[9px] font-black uppercase text-slate-400 dark:text-slate-300 tracking-wider">Carregar Arquivo</span>
                    <div className="border-2 border-dashed border-slate-200 dark:border-neutral-800 rounded-2xl p-4 bg-slate-50/20 dark:bg-neutral-900/20 hover:border-indigo-400 dark:hover:border-indigo-500 transition-all flex flex-col items-center justify-center text-center space-y-1 min-h-[120px]">
                      {customCoverUrl ? (
                        <div className="relative w-full h-24 rounded-lg overflow-hidden border border-slate-200 dark:border-neutral-800">
                        ...
                        <label className="w-full h-full cursor-pointer flex flex-col items-center justify-center py-2">
                          <Upload className="h-5 w-5 text-indigo-500 dark:text-indigo-400 animate-pulse mb-1" />
                          <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 block">Enviar Capa</span>
                          <span className="text-[8px] text-slate-400 dark:text-slate-500 font-medium">PNG, JPG (Máx. 2MB)</span>
                          <input
                            type="file"
                            accept="image/*"
                  ...
                  <div className="space-y-3">
                    <span className="text-[9px] font-black uppercase text-slate-400 dark:text-slate-300 tracking-wider">
                      {customCoverUrl ? 'Preview da Imagem' : 'Personalizar Capa por Cores'}
                    </span>
                    
                    {customCoverUrl ? (
                      <div className="bg-slate-50 dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center h-[120px] text-slate-400 dark:text-slate-300 text-xs font-semibold">
                        A imagem acima será usada como capa do treinamento.
                      </div>
                    ) : (
                      <div className="space-y-3 bg-slate-50/50 dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-2xl p-4">
                        {/* Live Cover Preview */}
                        <div 
                           style={{ backgroundColor: bgColor }} 
                          className="h-24 w-full rounded-xl flex items-center justify-center transition-all border border-slate-200/80 dark:border-neutral-800/80 shadow-inner relative overflow-hidden"
                        >
                          <span 
                            ...
                            {PRESET_COLORS.map((p) => (
                              <button
                                ...
                                className={`w-6 h-6 rounded-full border-2 cursor-pointer transition ${
                                  bgColor === p.bg ? 'border-slate-800 dark:border-slate-200 scale-110 shadow-sm' : 'border-white hover:border-slate-300'
                                }`}
                              />
                            ))}
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3 pt-1">
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                className="w-6 h-6 rounded border border-slate-300 dark:border-neutral-700 cursor-pointer p-0 bg-transparent"
                              />
                              <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">Cor de Fundo</span>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              <input
                                ...
                                className="w-6 h-6 rounded border border-slate-300 dark:border-neutral-700 cursor-pointer p-0 bg-transparent"
                              />
                              <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">Cor da Fonte</span>
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-slate-450 dark:text-slate-300 tracking-wider block">Descrição Geral e Ementa</label>
                <textarea
                  required
                  rows={5}
                  value={description}
                  ...
                  className="w-full text-xs font-semibold px-4 py-2.5 rounded-xl border border-slate-200 dark:border-neutral-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none leading-relaxed bg-slate-50/50 dark:bg-neutral-900/60 dark:text-white dark:placeholder-neutral-500"
                />
              </div>
            </div>

             {/* CARD 2: Course Content Structure (Multiple Lessons and Materials) */}
            <div className="bg-white dark:bg-[#0c0c0c] rounded-3xl border border-slate-200/80 dark:border-neutral-900 p-6 shadow-sm space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-neutral-800 pb-3">
                <div className="flex items-center gap-2">
                  <span className="p-1.5 bg-sky-50 dark:bg-neutral-900 text-sky-750 dark:text-sky-400 rounded-lg">
                    <FolderOpen className="h-4.5 w-4.5" />
                  </span>
                  <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm">Estrutura de Conteúdo do Treinamento</h3>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="p-1 bg-indigo-50 dark:bg-neutral-900 text-indigo-600 dark:text-indigo-400 rounded">
                        <Video className="h-3.5 w-3.5" />
                      </span>
                      <h4 className="text-[11px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Vídeo-aulas / Módulos ({lessons.length})</h4>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="space-y-4">
                    {lessons.length === 0 ? (
                      <div className="py-6 text-center text-slate-400 dark:text-slate-500 font-bold text-[10px] border border-dashed border-slate-200 dark:border-neutral-800 rounded-2xl bg-slate-50/40 dark:bg-neutral-900/20">
                        Nenhuma aula adicionada. Clique em "Nova Aula" para criar a grade técnica!
                      </div>
                    ) : (
                      lessons.map((ls, index) => (
                        <div key={ls.id} className="bg-slate-50 dark:bg-neutral-900/40 bg-opacity-70 p-4 rounded-2xl border border-slate-200 dark:border-neutral-800 space-y-3 relative">
                          <button
                            type="button"
                          ...
                          <div className="space-y-1 pr-6">
                            <span className="text-[9px] font-black uppercase text-indigo-605 dark:text-indigo-400 tracking-wider">Aula {index + 1}</span>
                            <input
                              type="text"
                              required
                              value={ls.title}
                              ...
                              className="w-full text-xs font-bold px-3 py-1.5 border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900/60 dark:text-white dark:placeholder-neutral-500 rounded-xl focus:outline-none focus:ring-1 focus:ring-indigo-500"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="text-[9px] font-bold text-slate-500 dark:text-slate-300 uppercase">Link do Vídeo (YouTube)</label>
                            <input
                              type="url"
                              required
                              value={ls.videoUrl}
                              ...
                              className="w-full text-xs font-semibold px-3 py-1.5 border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900/60 dark:text-white dark:placeholder-neutral-500 rounded-xl focus:outline-none focus:ring-1 focus:ring-indigo-500"
                            />
                          </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="p-1 bg-sky-50 dark:bg-neutral-900 text-sky-600 dark:text-sky-400 rounded">
                        <FileText className="h-3.5 w-3.5" />
                      </span>
                      <h4 className="text-[11px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Materiais de Apoio (PDFs) ({materials.length})</h4>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="space-y-4">
                    {materials.length === 0 ? (
                      ...
                      materials.map((mat, index) => (
                        <div key={mat.id} className="bg-slate-50 dark:bg-neutral-900/40 bg-opacity-70 p-4 rounded-2xl border border-slate-200 dark:border-neutral-800 space-y-3 relative">
                          <button
                            type="button"
                          ...
                          <div className="space-y-1 pr-6">
                            <span className="text-[9px] font-black uppercase text-sky-700 dark:text-sky-400 tracking-wider">Apostila {index + 1}</span>
                            <input
                              type="text"
                              required
                              value={mat.title}
                          ...
                          <div className="space-y-1.5">
                            <label className="text-[9px] font-bold text-slate-500 dark:text-slate-300 uppercase block">Arquivo PDF do Material</label>
                            {mat.pdfUrl ? (
                              <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-150 dark:border-emerald-900 rounded-xl p-2.5 flex items-center justify-between">
                                <span className="text-[10px] text-emerald-800 dark:text-emerald-300 font-bold truncate max-w-[180px]">
                              ...
                              <div>
                                <label className="flex flex-col items-center justify-center border-2 border-dashed border-sky-100 dark:border-neutral-800 rounded-xl py-3.5 bg-white dark:bg-neutral-900/60 hover:border-indigo-400 transition cursor-pointer">
            ...
            <div className="bg-white dark:bg-[#0c0c0c] rounded-3xl border border-slate-200/80 dark:border-neutral-900 p-6 shadow-sm space-y-6">
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-neutral-800 pb-3">
                <div className="flex items-center gap-2">
                  <span className="p-1.5 bg-amber-50 dark:bg-neutral-900 text-amber-705 dark:text-amber-400 rounded-lg">
                    <QuestionIcon className="h-4.5 w-4.5" />
                  </span>
                  <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm">Avaliação do Curso (Questões e Alternativas)</h3>
                </div>
                <button
                  type="button"
                  onClick={handleAddQuestion}
                  className="flex items-center gap-1 py-1.5 px-3 rounded-xl bg-slate-100 dark:bg-neutral-900 hover:bg-slate-200 dark:hover:bg-neutral-800 text-slate-700 dark:text-slate-300 font-bold text-[10px] uppercase transition"
                >
              ...
              <div className="space-y-6">
                {questions.length === 0 ? (
                  <div className="py-8 text-center text-slate-400 dark:text-slate-500 font-bold text-[11px] border-2 border-dashed border-slate-100 dark:border-neutral-800 rounded-3xl">
                  ...
                  questions.map((q, qIdx) => (
                    <div
                      key={q.id}
                      className="bg-slate-50/50 dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-2xl p-5 space-y-4 relative"
                    >
                      ...
                      <div className="space-y-1 pr-6">
                        <span className="text-[9px] font-black uppercase text-indigo-600 dark:text-indigo-400 tracking-wider">Questão {qIdx + 1} de {questions.length}</span>
                        <input
                          type="text"
                          required
                          value={q.text}
                          ...
                          className="w-full text-xs font-bold px-3 py-2 border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900/60 dark:text-white dark:placeholder-neutral-500 rounded-xl focus:outline-none focus:ring-1 focus:ring-indigo-500"
                        />
                      </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <div className="space-y-0.5">
                            <span className="text-[10px] font-black text-slate-800 dark:text-slate-100 uppercase tracking-wide block">Gabarito e Alternativas de Resposta</span>
                            <span className="text-[9px] text-slate-450 dark:text-slate-300 block">Selecione o seletor ou clique no badge para definir qual é a resposta correta da avaliação.</span>
                          </div>
                          <button
                            type="button"
                            onClick={() => handleAddAlternative(q.id)}
                            className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 bg-indigo-50 dark:bg-neutral-900 hover:bg-indigo-100 px-2.5 py-1 rounded-lg uppercase tracking-wider transition"
                          >
                              ...
                              <div
                                ...
                                className={`flex items-center gap-3 border rounded-xl px-3 py-2 text-xs transition duration-200 ${
                                  alt.isCorrect
                                    ...
                                    : 'border-slate-200 dark:border-neutral-800 hover:border-slate-400 bg-white dark:bg-neutral-900'
                                }`}
                              >
                                {/* Option Letter Indicator */}
                                <span className={`text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center ${
                                  alt.isCorrect ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-neutral-800 text-slate-500 dark:text-slate-400'
                                }`}>
                                  {optLetter}
                                </span>
                                ...
                                <label className="flex items-center cursor-pointer shrink-0">
                                  <input
                                    ...
                                    className="h-4 w-4 text-emerald-600 border-slate-300 dark:border-neutral-700 focus:ring-emerald-500 focus:outline-none cursor-pointer bg-white dark:bg-neutral-900"
                                  />
                                ...
                                <input
                                  ...
                                  className={`flex-1 text-xs border-0 bg-transparent py-0.5 focus:ring-0 focus:outline-none font-semibold ${
                                    alt.isCorrect ? 'text-emerald-950 dark:text-emerald-300' : 'text-slate-700 dark:text-slate-300'
                                  }`}
                                />
                                ...
                                <button
                                  ...
                                  className={`text-[9px] font-black uppercase px-2 py-1 rounded-md tracking-wider transition-all duration-200 cursor-pointer shrink-0 ${
                                    alt.isCorrect
                                      ...
                                      : 'bg-slate-100 dark:bg-neutral-800 text-slate-400 dark:text-slate-500 hover:bg-emerald-55 hover:text-emerald-700'
                                  }`}
                                >
                        ...
                        <div className="pt-1.5 border-t border-dashed border-slate-200/60 dark:border-neutral-800">
                          <label className="text-[9px] font-black text-slate-400 dark:text-slate-300 uppercase tracking-wider block mb-1">Explicação / Comentário da Questão (Mostrada ao aluno após responder)</label>
                          <input
                            type="text"
                            value={q.explanation || ''}
                            ...
                            className="w-full text-[11px] font-medium px-3.5 py-2 border border-slate-200 dark:border-neutral-800 bg-white dark:bg-neutral-900/60 rounded-xl focus:outline-none focus:ring-1 focus:ring-indigo-500 text-slate-600 dark:text-slate-300"
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-slate-50 dark:bg-neutral-900/40 border border-slate-100 dark:border-neutral-800 rounded-xl p-3 text-[10px] text-slate-400 dark:text-slate-500 font-bold flex gap-2">
              <AlertCircle className="h-4.5 w-4.5 text-slate-400 dark:text-slate-500 shrink-0 mt-0.5" />
            ...
            <button
              type="submit"
              className="w-full py-3 bg-slate-900 border border-slate-900 dark:bg-white dark:text-slate-950 dark:border-white text-white rounded-xl text-xs font-bold hover:bg-slate-800 dark:hover:bg-neutral-100 transition tracking-wide uppercase shadow cursor-pointer"
            >
